...
export function AdaptiveQuestionnaire({
  ...
  useEffect(() => {
    if (phase === 'preliminary' && !preliminaryResult && !isCalculating) {
      ...
      fetchPartialValuation(formData)
        ...
        .catch(err => {
          console.log('Backend partial valuation failed, using fallback:', err.message);
  ...
  }, [phase, formData, preliminaryResult, isCalculating]);

  // Update valuation in real-time during branch/closing phases
  useEffect(() => {
    if ((phase === 'branch' || phase === 'closing') && preliminaryResult) {
      const timer = setTimeout(() => {
        fetchPartialValuation(formData)
          .then(result => {
            setPreviousRange(prev => prev || result.range);
            setPreliminaryResult(result);
          })
          .catch(() => {
            // Silently fail - keep existing result
          });
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [formData, phase, questionIndex]);
  ...
  const validateCurrentQuestion = useCallback(() => {
    ...
    if ((question.type === 'number' || question.type === 'currency') && value) {
      ...
      if (question.max !== undefined && numValue > question.max) {
        return `Value must not exceed ₦${question.max.toLocaleString()}`;
      }
    }
  ...
  const getInputMode = (question: Question | BranchQuestion) => {
    if (question.type === 'number' || question.type === 'currency') return 'numeric';
    if (question.type === 'tel') return 'tel';
    return undefined;
  };
        ...
        <div className="mx-auto max-w-2xl">
          ...
          {isCalculating ? (
            <div className="mb-8 rounded-2xl bg-white p-12 text-center shadow-lg">
              ...
              <p className="text-lg text-gray-600">Calculating your preliminary valuation...</p>
            </div>
          ) : preliminaryResult ? (
            <>
              {/* Range Card */}
